...
@section('title', 'सबै दर्ता पत्रहरु')
@section('css')
...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>सबै दर्ता पत्रहरु</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">होम</a></li>
            <li class="breadcrumb-item active">सबै दर्ता पत्र सुची</li>
          </ol>
        </div>
      </div>
      ...
      <div class="row mb-2">
        <div class="col-sm-12">
          <button  class="btn btn-primary btn-all-darta">सबै पत्रहरु</button>
          <button  class="btn btn-primary btn-verified-darta">कार्यवाही भएका पत्रहरु</button>
          <button  class="btn btn-primary btn-unverified-darta">कार्यवाही नभएका पत्रहरु</button>
...
<script>
    ...
    var filter = localStorage.getItem('darta_filter') || 'all';
    var route;
    ...
    function setInitialRoute() {
        if (filter === 'verified') {
            route = "{{ route('admin.darta.verified-darta') }}";
            activateButton('.btn-verified-darta');
        } else if (filter === 'unverified') {
            route = "{{ route('admin.darta.unverified-darta') }}";
            activateButton('.btn-unverified-darta');
        } else {
            route = "{{ route('admin.darta.data-all') }}";
            activateButton('.btn-all-darta');
        }
    }
    ...
    function activateButton(selector) {
        ...
        $('.btn-all-darta, .btn-verified-darta, .btn-unverified-darta, .btn-ward')
    ...
    $('.btn-verified-darta').on('click', function() {
        route = "{{ route('admin.darta.verified-darta') }}";
        localStorage.setItem('darta_filter', 'verified');  // Save filter to localStorage
        activateButton('.btn-verified-darta');
        $('.ward-form').empty()
        reloadTable();
    });

    $('.btn-unverified-darta').on('click', function() {
        route = "{{ route('admin.darta.unverified-darta') }}";
        localStorage.setItem('darta_filter', 'unverified');  // Save filter to localStorage
        activateButton('.btn-unverified-darta');
        $('.ward-form').empty()
        reloadTable();
    });

    $('.btn-all-darta').on('click', function() {
        route = "{{ route('admin.darta.data-all') }}";
        localStorage.setItem('darta_filter', 'all');  // Save filter to localStorage
        activateButton('.btn-all-darta');
        $('.ward-form').empty()
        reloadTable();
    });
    ...
    $('.btn-ward').on('click', function() {
        route = "{{ route('admin.darta.ward-data') }}";
...
{{-- <script type="text/javascript">
    $(document).ready(function() {
        ...
        if (filter === 'verified') {
            route = "{{ route('admin.darta.verified-darta') }}";
            $('.btn-verified-darta').removeClass('btn-primary').addClass('btn-success');
            $('.btn-unverified-darta').removeClass('btn-success').addClass('btn-primary');

        } else if (filter === 'unverified') {
            route = "{{ route('admin.darta.unverified-darta') }}";
            $('.btn-unverified-darta').removeClass('btn-primary').addClass('btn-success');
            $('.btn-verified-darta').removeClass('btn-success').addClass('btn-primary');

        } else {
            route = "{{ route('admin.darta.data-all') }}";
            $('.btn-unverified-darta').removeClass('btn-primary').addClass('btn-success');
            $('.btn-verified-darta').removeClass('btn-success').addClass('btn-primary');
            $('.btn-all-darta').removeClass('btn-primary').addClass('btn-success');

        }

        // Save filter and reload table when verified button is clicked
        $('.btn-verified-darta').on('click', function(e) {
            $('.ward-form').empty();
        ...
        $('.btn-unverified-darta').on('click', function(e){
            $('.ward-form').empty();
            $(this).removeClass('btn-primary').addClass('btn-success');
            $('.btn-verified-darta').removeClass('btn-success').addClass('btn-primary');
            $('.btn-ward').removeClass('btn-success').addClass('btn-primary');
        ...
        $('.btn-ward').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.darta.ward-data') }}",
                type: "GET",
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}" // Include CSRF token for protection
                },
